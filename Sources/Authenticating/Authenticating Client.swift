...
extension Authenticating {
    ...
    public struct Client: Sendable {
        ...
        private let buildClient:
            @Sendable (@escaping @Sendable (API) throws -> URLRequest) -> ClientOutput
        ...
        public init(
            ...
            buildClient:
                @escaping @Sendable (@escaping @Sendable (API) throws -> URLRequest) ->
                ClientOutput
        ) {
...
extension AuthenticatingClient where Auth == BearerAuth {
    ...
    where
        Auth == BearerAuth, AuthRouter == BearerAuth.Router, APIRouter: TestDependencyKey,
...
extension AuthenticatingClient where Auth == BasicAuth {
    ...
    where
        Auth == BasicAuth, AuthRouter == BasicAuth.Router, APIRouter: TestDependencyKey,
        APIRouter.Value == APIRouter
    {
        @Dependency(APIRouter.self) var router
